...
def compute_parameters(vehicle, road_network, frames):  # reference track approach
    ...
    with tqdm(total=frames) as pbar:
        for _ in range(frames):
            ...
            parameters['acc2_magnitude'].append(vehicle.parallel_acc.norm())
...
def plot_parameter(ax, frames, parameter_list, parameter_name, limit=None):
    ...
    ax.text(0.95, 0.95, f"max acc={max_val:.2f}\nmin acc={min_val:.2f}", transform=ax.transAxes, ha='right',
            va='top', bbox=dict(facecolor='white', edgecolor='none', alpha=0.5))


def plot_parameters(vehicle, start, end, fig_name):
    acc = np.loadtxt("acc_magnitude.txt")
    acc2 = np.loadtxt("acc2_magnitude.txt")
    ...
    acc2 = acc2[start: end]
    resultant_acc = resultant_acc[start: end]
    fig, axs = plt.subplots(3, 2, figsize=(8, 10))
    axs = axs.flatten()
    frames = np.linspace(0, len(acc), len(acc))

    plot_parameter(axs[0], frames, acc, "Perpendicular Acceleration", vehicle.acc_limit)
    plot_parameter(axs[1], frames, error, "Error", math.radians(vehicle.angle_threshold))
    plot_parameter(axs[2], frames, acc2, "Parallel Acceleration", vehicle.acc_limit)
    plot_parameter(axs[3], frames, speed, "Speed")
    plot_parameter(axs[4], frames, resultant_acc, "Resultant Acceleration", vehicle.acc_limit)
...
def simulate(road_network, vehicle, frames, parameters, file_name):  # reference track
    ...
    vehicle_line, acc_line, acc2_line, velocity_line, closest_point, text, headed_line, track_line, trail_line, \
...
def write_parameters_to_file(arrays):
    write_parameter_to_file(arrays, "acc_magnitude")
    write_parameter_to_file(arrays, "acc2_magnitude")
